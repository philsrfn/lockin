...
describe('logWeight', () => {
  it('defaults to today and returns the entry with a fresh summary', async () => {
    const result = await logWeight(phil, { weightKg: 99.4 });
  ...
  it('upserts: stepping on the scale twice replaces the morning number', async () => {
    await logWeight(phil, { measuredOn: isoDaysAgo(0), weightKg: 99.4 });
    await logWeight(phil, { measuredOn: isoDaysAgo(0), weightKg: 99.1 });

    const entries = await listEntries(phil, 7);
  ...
  it.each([
    ...
    await expect(logWeight(phil, { weightKg: 99.4, ...override })).rejects.toThrow(message);
  });

  it('keeps numeric precision rather than handing back a string', async () => {
    const result = await logWeight(phil, { weightKg: 87.5 });
...
describe('listEntries', () => {
  it('returns oldest first, windowed by days', async () => {
    await logWeight(phil, { measuredOn: isoDaysAgo(40), weightKg: 101 });
    await logWeight(phil, { measuredOn: isoDaysAgo(3), weightKg: 99 });
    await logWeight(phil, { measuredOn: isoDaysAgo(1), weightKg: 98.6 });
...
describe('summary', () => {
  it('is empty rather than zeroed when he has never weighed in', async () => {
    const result = await summary(phil);
  ...
  it('carries the goal weight through from the profile', async () => {
    expect((await summary(phil)).goalWeightKg).toBe(80);
  });
  ...
  it('averages the last seven calendar days, not the last seven entries', async () => {
    ...
    await logWeight(phil, { measuredOn: isoDaysAgo(20), weightKg: 110 });
    await logWeight(phil, { measuredOn: isoDaysAgo(19), weightKg: 110 });
    await logWeight(phil, { measuredOn: isoDaysAgo(2), weightKg: 100 });
  ...
  it('reports week-over-week change as a loss when he is losing', async () => {
    for (let day = 13; day >= 0; day -= 1) {
      await logWeight(phil, { measuredOn: isoDaysAgo(day), weightKg: 100 - (13 - day) * 0.1 });
    }
  ...
  it('reaches further back than the requested window so early points have a full average', async () => {
    await logWeight(phil, { measuredOn: isoDaysAgo(35), weightKg: 102 });
    await logWeight(phil, { measuredOn: isoDaysAgo(30), weightKg: 101 });
    await logWeight(phil, { measuredOn: isoDaysAgo(0), weightKg: 99 });

    // 30 days of series, but the query fetches 44 so the oldest point is not
    // an average of one.
    const result = await summary(phil, 30);
